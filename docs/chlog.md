...
- StdQuotes 新增 `qfq_bars` / `hfq_bars` 自动复权K线方法
- Reader 新增 `validate` / `check_freshness` / `generate_report` 数据校验方法
- ExtQuotes 支持 `EX_CATEGORY` 枚举（港股/美股品种分类）
- Affair.parse 优先使用 opentdx HistoryFinancialCrawler 解析

### Refactor

- 财务列名（581项）下沉到 opentdx
- `_parse_date` 下沉到 opentdx utils
- `get_security_type` / `SECURITY_TYPE` / `SECURITY_COEFFICIENT` 继承自 opentdx
- tdxfinder 优先使用 opentdx TdxConnectCfgReader

### Chore

- opentdx 依赖版本约束 `>=0.5.0`
- 删除 reversion.py 死代码 `_reversion` / `baoli_qfq`

## v0.13.0

### Feat

- 统一从 opentdx 导入服务器列表和常量
- 新增港股/美股扩展市场品种分类支持
